...
module.exports = function () {
    ...
    files.forEach(item => {
        console.log(item.indexOf(apptype+'-pro-'));
        
        if (item.indexOf(apptype+'-pro-') > -1) {
            fs.removeSync('./zip/'+item)
        }
    })

    let version = config.environment.pro[mxpart[0]].version;
    version = version.split('.');
    version[2] = parseInt(version[2]) + 1;
    version = version.join('.');
    let build = config.environment.pro[mxpart[0]].build+1;
    config.environment.pro[mxpart[0]].version = version
    config.environment.pro[mxpart[0]].build = build

    let test = config.environment.pro[mxpart[0]];
    ...
    */
    ...
    global.packageName = '物资出厂' + apptype +'-pro-'+ version
}
