 ...
 */
function calcMax () {
    const parent = elScroller.value.parentNode
    const child = elScroller.value
...
const stopStep = 2
function scrollTo (_x:number, _y:number, _value = 1) {
    scrollToX = _x
    scrollToY = _y
    const _dx = scrollX - _x
    const _dy = scrollY - _y
...
function step () {
    ...
    // 当快要滚动到指定点的Y轴时
    const arriveY = scrollToY !== null && ((stepY < 0 && _scrollY > scrollToY) || (stepY > 0 && _scrollY < scrollToY))
    ...
    const arriveX = scrollToX !== null && ((stepX < 0 && _scrollX > scrollToX) || (stepX > 0 && _scrollX < scrollToX))
    ...
    if ((_scrollY < 0 && props.topBounce) || (_scrollY >= maxY && props.bottomBounce)) {
        // 是否回弹
        const _isBounce = (stepY < 0 && scrollY < 0) || (stepY > 0 && scrollY > maxY)
    ...
    if ((_scrollX < 0 && props.leftBounce) || (_scrollX >= maxX && props.rightBounce)) {
        // 是否回弹
        const _isBounce = (stepX < 0 && scrollX < 0) || (stepX > 0 && scrollX > maxX)
        if (!_isBounce) {
...
function calcMoveSpeed () {
    ...
    const num = _touchList.length
    if (num > 30) {
        _touchList = _touchList.slice(num - 30, num)
    }
    const last = _touchList.length - 1
    let first = 0
    let a = 0
...
function onTouchmove (e:TouchEvent) {
    ...
    if (_touches.length == null) {
        throw new Error(`Invalid touch list: _${_touches}`)
    }

    // 两只手指滑动处理中心点
    let _moveX, _moveY
    const isSingleTouch = _touches.length === 1
    if (isSingleTouch) {
    ...
    const _upxy = touchMoveList[touchMoveList.length - 1]
    moveX = _upxy.x
    moveY = _upxy.y
    ...
    // 判断滑动方向，并获取滑动距离
    const res = getDirection(moveX, moveY, _moveX, _moveY, !!direction)
