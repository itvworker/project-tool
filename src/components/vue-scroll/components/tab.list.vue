  ...
  export default {
    ...
    methods: {
      resize() {
        ...
        this.scroller.setDimensions(container.clientWidth, container.clientHeight, content.offsetWidth, content.offsetHeight-60);
      },
      ...
      touchMove(e) {
        ...
        let maxh = this.scroller.__maxScrollTop;
        if(top <= 0){
            this.$emit('status', 0);
        }else if(top >= maxh){
            this.$emit('status', 1);
